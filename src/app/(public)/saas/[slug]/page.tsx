...
import {
  getPublishedSaaSBySlug,
  getPublishedSaaSStaticParams,
  getRelatedSaaSProducts,
  saasMetadata,
} from "@/modules/saas/public"
import { jsonArray, priceLabel } from "@/modules/saas/utils"

export const revalidate = 86400
export const dynamicParams = true

export async function generateStaticParams() {
  const products = await getPublishedSaaSStaticParams()
...
export default async function SaaSDetailPage({
  params,
}: {
  params: Promise<{ slug: string }>
}) {
  ...
  const related = await getRelatedSaaSProducts(product.id, product.category)
  const productUrl = new URL(`/saas/${product.slug}`, site.url).toString()
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "SaaS", item: new URL("/saas", site.url).toString() },
      ...
      <Container className="py-10">
        ...
        {related.length > 0 ? (
          <section className="mt-16 border-t pt-12">
            <h2 className="text-2xl font-semibold tracking-tight">Related SaaS Products</h2>

...
export default async function ComparisonDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Vergleiche", item: new URL("/compare", site.url).toString() },
  ...
  return (
    <>
      ...
      <Container className="py-10">
        <Breadcrumb items={[{ label: "Startseite", href: "/" }, { label: "Vergleiche", href: "/compare" }, { label: comparison.title }]} />
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-[1fr_260px]">
          <div className="space-y-8">
            ...
            <ListSection title={`Geeignet für ${comparison.itemAName}`} items={jsonArray(comparison.bestForA)} />
            <ListSection title={`Geeignet für ${comparison.itemBName}`} items={jsonArray(comparison.bestForB)} />
            ...
            <div className="flex flex-wrap gap-3 rounded-lg border bg-card p-5">
              <AffiliateButton href={comparison.itemAAffiliateUrl ?? comparison.itemAUrl}>{comparison.itemAName} besuchen</AffiliateButton>
              <AffiliateButton href={comparison.itemBAffiliateUrl ?? comparison.itemBUrl}>{comparison.itemBName} besuchen</AffiliateButton>
            </div>
